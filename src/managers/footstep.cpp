...
namespace {
	...
	float VolumeFunction(float scale, const VolumeParams& params) {
		float k = params.k;
		float a = params.a;
		float n = params.n;
		float s = params.s;
	...
	float FrequencyFunction(float scale, const VolumeParams& params) {
		float a = params.a;
		return soft_core(scale, 0.01, 1.0, 1.0, a, 0.0)*0.5+0.5;
	}

	float FalloffFunction(NiAVObject* source) {
		if (source) {
	...
	BSSoundHandle GetSound(NiAVObject* foot, const float& scale, BSISoundDescriptor* sound_descriptor, const VolumeParams& params, std::string_view tag) {
		BSSoundHandle result = BSSoundHandle::BSSoundHandle();
		auto audio_manager = BSAudioManager::GetSingleton();
		if (sound_descriptor && foot && audio_manager) {

			float volume = VolumeFunction(scale, params);
			float frequency = FrequencyFunction(scale, params);
	...
	BSISoundDescriptor* GetLFootstepSounddesc(const Foot& foot_kind) {
		switch (foot_kind) {
			case Foot::Left:
			case Foot::Front:
	...
	BSISoundDescriptor* GetLJumpLandSounddesc(const Foot& foot_kind) {
		switch (foot_kind) {
			case Foot::JumpLand:
	...
	BSISoundDescriptor* GetXlFootstepSounddesc(const Foot& foot_kind) {
		switch (foot_kind) {
			case Foot::Left:
			case Foot::Front:
	...
	BSISoundDescriptor* GetXlRumbleSounddesc(const Foot& foot_kind) {
		switch (foot_kind) {
			case Foot::Left:
			case Foot::Front:
	...
	BSISoundDescriptor* GetXlSprintSounddesc(const Foot& foot_kind) {
		switch (foot_kind) {
			case Foot::Left:
			case Foot::Front:
	...
	BSISoundDescriptor* GetXxlFootstepSounddesc(const Foot& foot_kind) {
		switch (foot_kind) {
			case Foot::Left:
			case Foot::Front:
