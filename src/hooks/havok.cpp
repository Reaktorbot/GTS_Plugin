...
namespace Hooks
{
	...
	Hook_Havok::CollisionFilterComparisonResult Hook_Havok::CompareFilterInfo(RE::bhkCollisionFilter* a_collisionFilter, uint32_t a_filterInfoA, uint32_t a_filterInfoB)
	{
		...
		if ((layerA == COL_LAYER::kBiped || layerA == COL_LAYER::kBipedNoCC || layerA == COL_LAYER::kCharController) && (layerB == COL_LAYER::kBiped || layerB == COL_LAYER::kBipedNoCC || layerB == COL_LAYER::kCharController)) {
			// Biped vs. biped
